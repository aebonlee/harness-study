  ...
  return (
    <div className="guide-section">
      <div className="guide-content-header">
        <h1>{isKo ? '스킬이란?' : 'What are Skills?'}</h1>
        <p>{isKo ? 'Harness 스킬의 개념과 에이전트 효율화에서의 역할을 이해합니다.' : 'Understand the concept of Harness skills and their role in agent efficiency.'}</p>
      </div>
...
function DisclosureSection({ isKo }: { isKo: boolean }): ReactElement {
  return (
    <div className="guide-section">
      <div className="guide-content-header">
        <h1>{isKo ? '프로그레시브 디스클로저' : 'Progressive Disclosure'}</h1>
        <p>{isKo ? '컨텍스트 창을 효율적으로 사용하는 핵심 스킬 설계 기법을 마스터합니다.' : 'Master the key skill design technique for efficiently using the context window.'}</p>
      </div>
      <h2>{isKo ? '프로그레시브 디스클로저란?' : 'What is Progressive Disclosure?'}</h2>
      <p>{isKo ? '프로그레시브 디스클로저(Progressive Disclosure)는 모든 정보를 한 번에 제공하는 대신, 필요한 시점에 필요한 정보만 단계적으로 공개하는 설계 원칙입니다. 이를 통해 에이전트의 컨텍스트 창을 낭비 없이 사용할 수 있습니다.' : 'Progressive Disclosure is a design principle that provides only the necessary information at the right time, rather than providing all information at once. This allows efficient use of the agent\'s context window without waste.'}</p>
      <h3>{isKo ? '3단계 디스클로저 구조' : '3-Level Disclosure Structure'}</h3>
      <ol>
        <li>
          <strong>{isKo ? '레벨 1: 요약 (Summary)' : 'Level 1: Summary'}</strong>
          <p>{isKo ? '스킬의 핵심 목적과 트리거만 포함. 항상 로드됩니다 (1-3줄). 에이전트가 이 스킬이 필요한지 판단하는 데 사용합니다.' : 'Contains only the skill\'s core purpose and trigger. Always loaded (1-3 lines). Used by the agent to determine if this skill is needed.'}</p>
        </li>
        <li>
          <strong>{isKo ? '레벨 2: 개요 (Overview)' : 'Level 2: Overview'}</strong>
          <p>{isKo ? '스킬 활성화 시 로드. 주요 단계와 중요 지침 포함 (10-30줄). 대부분의 작업에 충분한 정보를 제공합니다.' : 'Loaded when the skill is activated. Contains major steps and important guidelines (10-30 lines). Provides sufficient information for most tasks.'}</p>
        </li>
        <li>
          <strong>{isKo ? '레벨 3: 상세 (Details)' : 'Level 3: Details'}</strong>
...
function StructureSection({ isKo }: { isKo: boolean }): ReactElement {
  return (
    <div className="guide-section">
      <div className="guide-content-header">
...
function TriggersSection({ isKo }: { isKo: boolean }): ReactElement {
  return (
    <div className="guide-section">
      <div className="guide-content-header">
...
function OptimizeSection({ isKo }: { isKo: boolean }): ReactElement {
  return (
    <div className="guide-section">
      <div className="guide-content-header">
        <h1>{isKo ? '스킬 최적화' : 'Skill Optimization'}</h1>
...
function TestSection({ isKo }: { isKo: boolean }): ReactElement {
  return (
    <div className="guide-section">
      <div className="guide-content-header">
        <h1>{isKo ? '스킬 테스트' : 'Skill Testing'}</h1>
        <p>{isKo ? '스킬이 올바르게 동작하는지 검증하는 테스트 방법론을 알아봅니다.' : 'Learn the testing methodology for verifying skills work correctly.'}</p>
      </div>
      <h2>{isKo ? '스킬 테스트 접근법' : 'Skill Testing Approach'}</h2>
      <h3>{isKo ? '1. 트리거 검증' : '1. Trigger Verification'}</h3>
      <p>{isKo ? '스킬이 의도한 상황에서 올바르게 트리거되는지, 그리고 의도하지 않은 상황에서는 트리거되지 않는지 테스트합니다.' : 'Test that the skill triggers correctly in intended situations and does not trigger in unintended situations.'}</p>
      <h3>{isKo ? '2. A/B 테스트' : '2. A/B Testing'}</h3>
