...
3. 보안 취약점 및 성능 이슈 식별
4. 우선순위별 개선 제안 정리`,
    codeEn: `# CLAUDE.md — Global guidelines (always applied)
## Tech Stack
- React 19 + TypeScript + Vite
## Coding Rules
...
4. Compile prioritized improvement suggestions`,
    codeLang: 'markdown' },
  /* 스킬 */
  { category: 'skill',
...
1. 코드 확인
2. 리뷰 작성`,
    codeEn: `# ✅ Recommended: 2-3 sub-guidelines per step
...
- TypeScript 파일 10줄 이상 수정 시 /review 권장`,
    codeEn: `# commands/review.md
...
- Suggest /review when 10+ TypeScript file lines are changed`,
    codeLang: 'markdown' },
  /* 팀 & 패턴 */
  { category: 'team',
...
# Step 2: 결과 취합 (순차 실행)
모든 Task 완료 → 결과 통합 → 품질 검토 → 최종본`,
    codeEn: `# Fan-out pattern — reduce time via parallel processing
...
독립 + 순차 혼재?
  └─→ 두 패턴 조합
       팬아웃(병렬 조사) → 취합 → 파이프라인(순차 집필)`,
    codeEn: `Pattern Selection Decision Tree
────────────────────────────────────────────
...
Mixed independent + sequential?
  └─→ Combine both patterns
       fan-out (parallel research) → merge → pipeline (sequential writing)`,
    codeLang: 'text' },
  /* 기타 */
  { category: 'etc',
  ...
  /* 기초 추가 */
  { category: 'basic',
    q: '오케스트레이터는 별도의 프로그램인가요?',
    qEn: 'Is the orchestrator a separate program?',
    a: '아닙니다. 오케스트레이터는 Claude Code 세션 자체입니다. CLAUDE.md에 "너는 오케스트레이터 역할을 한다"고 정의하고 Task 도구 사용 권한을 부여하면, 그 세션이 오케스트레이터로 동작합니다. 별도 서버나 소프트웨어 없이 마크다운 파일만으로 구현됩니다.',
    aEn: 'No. The orchestrator is the Claude Code session itself. Define "you are the orchestrator" in CLAUDE.md and grant Task tool permissions — that session acts as the orchestrator. No separate server or software needed, just markdown files.' },
  { category: 'basic',
    q: 'Harness와 LangChain, AutoGPT 같은 도구의 차이점은?',
    qEn: 'How is Harness different from LangChain or AutoGPT?',
    a: 'LangChain, AutoGPT는 코드로 에이전트를 구성하는 프레임워크입니다. Harness는 마크다운 파일(스킬)로 에이전트를 정의합니다. 코딩 없이 구현 가능하고 버전 관리가 쉽습니다. 반면 복잡한 분기·루프 로직은 코드 기반 프레임워크가 더 유연합니다.',
    aEn: 'LangChain and AutoGPT are code-based agent frameworks. Harness defines agents through markdown skill files — no coding required and easy to version-control. However, code-based frameworks are more flexible for complex branching and loop logic.' },
  /* 스킬 추가 */
  { category: 'skill',
    q: '스킬 트리거가 원하는 상황에서 발동되지 않으면?',
    qEn: 'What if a skill trigger doesn\'t fire when expected?',
    a: '트리거 문구가 너무 좁거나 Claude가 다른 스킬을 선택했을 수 있습니다. 해결 방법: ① 트리거에 동의어/유사 표현 추가, ② /skill-name으로 명시적 호출, ③ CLAUDE.md 전역 규칙에 "코드 리뷰 요청 시 반드시 /review 사용"처럼 명시. 가장 확실한 방법은 명시적 슬래시 커맨드입니다.',
    aEn: 'The trigger phrase may be too narrow, or Claude may have selected a different skill. Solutions: ① Add synonyms to the trigger, ② Use an explicit /skill-name call, ③ Add to CLAUDE.md: "Always use /review for code review requests." Explicit slash commands are the most reliable approach.' },
  { category: 'skill',
    q: '여러 프로젝트에서 같은 스킬을 공유하려면?',
    qEn: 'How do I share the same skill across multiple projects?',
    a: '두 가지 방법이 있습니다. ① 전역 설정: ~/.claude/commands/에 스킬을 저장하면 모든 프로젝트에서 사용 가능합니다. ② Git 서브모듈: 공통 스킬만 별도 저장소로 분리하고 각 프로젝트에서 서브모듈로 연결합니다. 조직 전체 공유에는 서브모듈 방식이 적합합니다.',
    aEn: 'Two approaches: ① Global config: Save skills in ~/.claude/commands/ to use across all projects. ② Git submodule: Separate common skills into a dedicated repo and link each project via submodule. The submodule approach is better for organization-wide sharing.' },
  { category: 'skill',
    q: '스킬 파일에 조건 분기 로직을 넣을 수 있나요?',
    qEn: 'Can I add conditional branching logic to skill files?',
    a: '마크다운은 프로그래밍 언어가 아니어서 if/else 구문은 없습니다. 하지만 "입력이 TypeScript이면 A를 하고, Python이면 B를 하라"처럼 자연어로 조건을 기술할 수 있습니다. Claude가 이를 이해하고 상황에 맞게 행동합니다. 복잡한 분기가 많다면 스킬을 여러 개로 나누는 것이 더 명확합니다.',
    aEn: 'Markdown isn\'t a programming language, so there\'s no if/else syntax. But you can describe conditions naturally: "if TypeScript, do A; if Python, do B." Claude understands this and acts accordingly. For many complex branches, splitting into multiple skill files is clearer.' },
  /* 팀 & 패턴 추가 */
  { category: 'team',
    q: '에이전트 팀의 적정 규모는 몇 명인가요?',
    qEn: 'What is the right size for an agent team?',
    a: '"2-Pizza Rule"을 적용하세요. 에이전트가 2-6개일 때 가장 효율적입니다. 2개 미만은 팀의 의미가 없고, 7개 이상이면 조율 비용이 급격히 올라갑니다. 처음에는 오케스트레이터 + 2-3개 서브에이전트로 시작하고 필요에 따라 늘리세요.',
    aEn: 'Apply the "2-Pizza Rule." Teams of 2-6 agents are most efficient. Fewer than 2 has no team benefit; 7+ makes coordination costs skyrocket. Start with orchestrator + 2-3 subagents and scale as needed.' },
  { category: 'team',
    q: '오케스트레이터에는 어떤 모델을 써야 하나요?',
    qEn: 'What model should I use for the orchestrator?',
    a: '오케스트레이터는 복잡한 판단(작업 분해, 에이전트 선택, 결과 평가)을 담당하므로 가장 강력한 모델(Claude Opus)을 권장합니다. 서브에이전트는 작업 복잡도에 따라 Sonnet(중간 작업), Haiku(단순 작업)를 사용해 비용을 최적화하세요.',
    aEn: 'The orchestrator handles complex judgments (task decomposition, agent selection, result evaluation), so the most capable model (Claude Opus) is recommended. Use Sonnet for medium-complexity tasks and Haiku for simple tasks in subagents to optimize costs.' },
  { category: 'team',
    q: '팬아웃 후 여러 에이전트의 결과를 어떻게 취합하나요?',
    qEn: 'How do I aggregate results from multiple agents after fan-out?',
    a: '세 가지 패턴이 있습니다. ① 파일 기반: 각 에이전트가 결과를 파일에 저장하고 오케스트레이터가 읽어 통합합니다. ② 직접 반환: 오케스트레이터가 각 Task의 반환값을 수집합니다. ③ 중간 요약: 각 에이전트가 긴 결과를 요약해서 반환합니다. 결과 크기가 크면 파일 기반이 컨텍스트 절약에 유리합니다.',
    aEn: 'Three patterns: ① File-based: Each agent saves results to files; orchestrator reads and integrates. ② Direct return: Orchestrator collects each Task\'s return value. ③ Intermediate summary: Each agent returns a summarized result. File-based is better for context conservation with large results.',
    codeKo: `# ① 파일 기반 취합 (권장 — 컨텍스트 절약)
...
# ③ 중간 요약 (대용량 처리 시)
각 에이전트가 결과를 3줄 핵심 요약으로 압축 반환`,
    codeEn: `# ① File-based aggregation (recommended — saves context)
...
Each agent returns a 3-line compressed key summary`,
    codeLang: 'text' },
  /* 기타 추가 */
  { category: 'etc',
    q: 'Harness 사용 시 API 비용을 줄이는 방법은?',
    qEn: 'How can I reduce API costs when using Harness?',
    a: '① 모델 계층화: 오케스트레이터 Opus, 작업 에이전트 Sonnet, 단순 작업 Haiku. ② 컨텍스트 압축: 중간 요약으로 불필요한 이력 제거. ③ 스킬 최적화: 프로그레시브 디스클로저로 스킬 파일 크기 최소화. ④ 결과 캐싱: 반복적인 연구·분석 결과를 파일에 저장해 재사용. 이 방법들을 조합하면 비용을 30-50% 절감할 수 있습니다.',
    aEn: '① Model tiering: Opus for orchestrator, Sonnet for task agents, Haiku for simple tasks. ② Context compression: Remove unnecessary history with intermediate summaries. ③ Skill optimization: Minimize skill file sizes via progressive disclosure. ④ Result caching: Store repetitive research results in files for reuse. Combining these can reduce costs by 30-50%.' },
  { category: 'etc',
    q: '에이전트가 같은 실수를 반복하면 어떻게 하나요?',
    qEn: 'What do I do if an agent repeatedly makes the same mistake?',
    a: '반복 실수는 스킬 파일 지침이 불명확하다는 신호입니다. ① 해당 단계에 "절대로 ~하지 말 것" 형태의 명시적 금지사항 추가. ② 올바른 예시와 잘못된 예시를 함께 포함. ③ CLAUDE.md 전역 규칙에 반복 실수 패턴 추가. 그래도 지속되면 그 작업 단계를 더 작은 단위로 분해하세요.',
    aEn: 'Repeated mistakes signal unclear skill file instructions. ① Add explicit prohibitions: "Never do X." ② Include both correct and incorrect examples. ③ Add the mistake pattern to CLAUDE.md global rules. If it persists, break down that task step into smaller units.' },
  { category: 'etc',
    q: 'CLAUDE.md가 너무 길어지면 어떻게 관리하나요?',
    qEn: 'How do I manage CLAUDE.md when it gets too long?',
    a: 'CLAUDE.md가 200줄을 넘으면 분리를 고려하세요. 기본 원칙(프로젝트 목적, 기술 스택, 코딩 컨벤션)만 CLAUDE.md에 유지하고, 특정 작업 지침은 commands/ 폴더의 스킬 파일로 이동합니다. docs/ 폴더에 상세 문서를 만들고 CLAUDE.md에서 @docs/guide.md 형태로 참조하는 방법도 효과적입니다.',
    aEn: 'Consider splitting when CLAUDE.md exceeds 200 lines. Keep only fundamentals (project purpose, tech stack, conventions) in CLAUDE.md; move specific task instructions to skill files in commands/. Creating detailed docs in a docs/ folder and referencing them as @docs/guide.md in CLAUDE.md is also effective.' },
];
                  ...
                  {item.codeKo && (
                    <div className="code-block" style={{ marginTop: '1rem' }}>
                      <div className="code-block-header">
                        <span className="code-block-lang">{item.codeLang ?? 'text'}</span>
                      </div>
                      <div className="code-block-body">
                        <pre><code>{isKo ? item.codeKo : (item.codeEn ?? item.codeKo)}</code></pre>
                      </div>
                    </div>
                  )}
                </div>
