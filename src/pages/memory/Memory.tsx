  ...
  3. context_usage > 80%면 checkpoint 저장 후 체인지오버`}</code></pre>
      ...
      <div style={{ display: 'flex', justifyContent: 'center', margin: '2rem 0' }}>
        <svg viewBox="0 0 480 260" style={{ maxWidth: 480, width: '100%' }} role="img" aria-label={isKo ? '컨텍스트 창 구성 다이어그램' : 'Context Window Composition Diagram'}>
          {/* Container */}
          <rect x="10" y="10" width="460" height="220" rx="12" fill="none" stroke="var(--color-text)" strokeWidth="1.5" strokeDasharray="4 2" />
          ...
          <text x="74" y="95" textAnchor="middle" fontSize="10" fontWeight="600" fill="var(--color-text)">10%</text>
          {/* Conversation bar */}
          ...
          <text x="172" y="95" textAnchor="middle" fontSize="10" fontWeight="600" fill="var(--color-text)">20%</text>
          ...
          <text x="291" y="102" textAnchor="middle" fontSize="10" fill="var(--color-text)">Read, Search...</text>
          {/* Skill Files bar */}
          ...
          <text x="410" y="95" textAnchor="middle" fontSize="10" fontWeight="600" fill="var(--color-text)">10%</text>
          {/* Free space */}
          <rect x="30" y="120" width="424" height="25" rx="6" fill="var(--color-bg-secondary)" stroke="var(--color-border)" strokeWidth="1.5" />
          <text x="242" y="137" textAnchor="middle" fontSize="10" fill="var(--color-text)">{isKo ? '여유 공간 20% — 이 공간이 부족하면 망각 시작' : 'Free space 20% — Forgetting starts when this runs out'}</text>
          {/* Optimization arrows */}
          <text x="30" y="175" fontSize="11" fontWeight="600" fill="var(--color-primary)">{isKo ? '최적화 전략:' : 'Optimization:'}</text>
          <text x="30" y="195" fontSize="10" fill="var(--color-text)">{isKo ? '① 프로그레시브 디스클로저 → 스킬 파일 최소화' : '① Progressive disclosure → minimize skill files'}</text>
          <text x="30" y="212" fontSize="10" fill="var(--color-text)">{isKo ? '② 중간 요약 → 대화 이력 압축' : '② Intermediate summary → compress history'}</text>
          <text x="30" y="229" fontSize="10" fill="var(--color-text)">{isKo ? '③ 선택적 읽기 → 도구 결과 최소화' : '③ Selective reading → minimize tool results'}</text>
        </svg>
      </div>
